...
def log_trainable_parameters(vla, sequence_aggregation_student=None, distillation_loss_fn=None):
    ...
    print(f"  - All params: {lora_all_params:,}")
    print(f"  - Trainable %: {100 * lora_trainable_params / lora_all_params:.4f}%")
    print("")
    print(f"Distillation Modules (MLP + Loss):")
    print(f"  - Trainable params: {distill_trainable_params:,}")
    print(f"  - All params: {distill_all_params:,}")
    print(f"  - Trainable %: {100.0 if distill_all_params > 0 else 0.0:.4f}%")
    print("")
    print(f"TOTAL:")
...
class FinetuneConfig:
    ...
    bottleneck_dim: int = 2048                                      # MLP bottleneck dimension for student projection
...
def save_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        # Save training config as YAML in checkpoint_dir (for evaluation logging)
        import yaml
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if distributed_state.is_main_process:
        ...
        print(f"  - Weight Decay Enabled: {cfg.distill_weight_decay_enabled}")
        if cfg.distill_weight_decay_enabled:
            print(f"    * Decay Start Step: {cfg.distill_weight_decay_start_step}")
            print(f"    * Decay End Step: {cfg.distill_weight_decay_end_step}")
            print(f"    * Final Ratio: {cfg.distill_weight_decay_final_ratio}")
            print(f"    * Final Weight: {cfg.distill_weight * cfg.distill_weight_decay_final_ratio:.6f}")
