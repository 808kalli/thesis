...
class LAPAHiddenStateExtractor:
    ...
    def extract_hidden_state_from_image(self, image: Image.Image, task_description: str = None) -> np.ndarray:
        ...
        # STEP 5: Extract hidden states from final layer (text + vision + continuation)
        # hidden_states[-1] is the final layer output AFTER RMSNorm (normalized)
        if hasattr(outputs, 'hidden_states') and outputs.hidden_states is not None:
            # outputs.hidden_states[-1] = final transformer layer AFTER RMSNorm
